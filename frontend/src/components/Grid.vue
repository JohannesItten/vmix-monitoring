...
const startSocket = (wsURL, waitTimer, waitSeed, multiplier) => {
  ...
  socket.onopen = (event) => {
    ...
    socket.send(JSON.stringify({ type: 'watch', payload: { page: currentPage } }))
    waitTimer = waitSeed
  }
  ...
  socket.onclose = (event) => {
    ...
    setTimeout(() => {
      startSocket(socket.url, waitTimer, waitSeed, multiplier)
    }, waitTimer)
  }
...
let serverURI = 'ws://' + location.hostname + ':9090/ws'
